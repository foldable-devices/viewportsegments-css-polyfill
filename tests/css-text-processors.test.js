...
const { _processSpanningMediaBlock, _getMediaFeatures, replaceCSSEnvVariables, _getMediaTypes } = require("../src/utils/css-text-processors.js");
...
test(`_processSpanningMediaBlock should find media query blocks starting with @media and containing ${SPANNING_MF_KEY} `, function (t) {
  ...
  t.equal(spanningmediaQuery, _processSpanningMediaBlock(spanningmediaQuery + otherCSSText)[0][0]);
  t.end();
});
...
test('_processSpanningMediaBlock should find ALL spanning media blocks', function (t) {
  ...
  let otherCSSText = `body{ background: blue' }`
  t.equal(spanningmediaQuery2, _processSpanningMediaBlock(spanningmediaQuery +  spanningmediaQuery2 + otherCSSText)[1][0]);
  t.end();
});
...
test('_processSpanningMediaBlock should capture the media query defintion', function (t) {
  let spanningmediaQuery = `@media type (${SPANNING_MF_KEY}: single-fold-vertical) and (min-width: 900px){body { color: green; }}`;
  let otherCSSText = `body{ background: blue' }`
  t.equal('@media type (spanning: single-fold-vertical) and (min-width: 900px)', _processSpanningMediaBlock(spanningmediaQuery + otherCSSText)[0][1]);
  t.end();
});
...
test('_processSpanningMediaBlock should capture the media query CSS content', function (t) {
  ...
  let otherCSSText = `body{ background: blue' }`
  t.equal('body { color: green; }', _processSpanningMediaBlock(spanningmediaQuery + otherCSSText)[0][2]);
  t.end();
});
...
test('replaceCSSEnvVariables should find and replace  all istances of an CSS env(variable)', function (t) {
  ...
  `;
  t.equal(expectedOutput, replaceCSSEnvVariables(sheetStr, 'pre-defined-variable', '99px'));
  t.end();
});
